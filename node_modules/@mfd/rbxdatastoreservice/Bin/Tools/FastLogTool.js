...
const logsFolder = process.platform === 'win32'
    ? (0, path_1.join)(process.env.LOCALAPPDATA, 'MFDLABS', 'Logs')
    : (0, path_1.join)(process.env.HOME, '.cache', 'mfdlabs', 'log');
...
const DFFlag = function (name) {
    ...
    if (df) {
        ...
        return exports.cache[name] || false;
    }
...
const DFString = function (name) {
    ...
    if (df) {
        ...
        return exports.cache[name] || null;
    }
    return df[name] || exports.cache.DFString[name] || null;
};
...
function setUpFLog() {
    ...
    if (df !== null)
        for (const k in df)
            exports.DFLog[k] = df[k];
    if (sf !== null)
        for (const k in sf)
            exports.SFLog[k] = sf[k];
    if (ff !== null)
        for (const k in ff)
            exports.FFlag[k] = ff[k];
    if (dff !== null)
        for (const k in dff)
            exports.DFFlag[k] = dff[k];
    if (sff !== null)
        for (const k in sff)
            exports.SFFlag[k] = sff[k];
    if (fi !== null)
        for (const k in fi)
            exports.FInt[k] = fi[k];
    if (dfi !== null)
        for (const k in dfi)
            exports.DFInt[k] = dfi[k];
    if (sfi !== null)
        for (const k in sfi)
            exports.SFInt[k] = sfi[k];
    if (fs !== null)
        for (const k in fs)
            exports.FString[k] = fs[k];
    if (dfs !== null)
        for (const k in dfs)
            exports.DFString[k] = dfs[k];
    if (sfs !== null)
        for (const k in sfs)
            exports.SFString[k] = sfs[k];
    if (fss !== null && Array.isArray(fss))
        fss.forEach((e) => exports.FSettings.push(e));
...
function printMessage(level, threadId, timeStamp, message, arg0, arg1, arg2, arg3, arg4) {
    if (FFLags['FastLogEnabled']) {
        if (!(0, fs_1.existsSync)(logsFolder))
            (0, fs_1.mkdirSync)(logsFolder, { recursive: true });
        ...
        console.log(out);
        if (FFLags['LogToFile']) {
            const logFile = (0, path_1.join)(logsFolder, FFLags['UseOneLog'] ? 'main-log.log' : `${process.pid.toString(16)}.log`);
            fs_1.default.appendFileSync(logFile, `${out}\n`, {
                encoding: 'utf-8',
            });
        }
    }
